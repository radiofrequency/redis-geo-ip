...
static int geohash_encode_impl(double latitude, double longitude, char* r, size_t capacity){
        ...
        for(int i=0; i<8; i++){
                ...
                //printf("interleaved: %llu", interleaved[7-1]);
        }
      ...
      //  }
        ...
        //uint64_t done = ((unsigned PY_LONG_LONG)interleaved[0]<<48) + ((unsigned PY_LONG_LONG)interleaved[1]<<32) + ((unsigned PY_LONG_LONG)interleaved[2]<<16) + (unsigned PY_LONG_LONG)interleaved[3];
     //   uint64_t done2 = ((unsigned PY_LONG_LONG)interleaved[4]<<48) + ((unsigned PY_LONG_LONG)interleaved[5]<<32) + ((unsigned PY_LONG_LONG)interleaved[6]<<16) + (unsigned PY_LONG_LONG)interleaved[7];
       ...
       // printf("\n");
        ...
        //printf("new interleaved: %llu\n", done);
        //printf("new interleaved2: %llu", done2);
        int ret = GEOHASH_OK;
...
static int geohash_decode_impl(char* r, size_t length, double *latitude, double *longitude){
        ...
        for(int i=0; i<8; i++){
                uint8_t upper, lower;
     //             printf("interleaved%d: %llu\n", i,interleaved[i]);

                deinterleave(interleaved[i], &upper, &lower);
                lon64 = (lon64<<8)+upper;
                lat64 = (lat64<<8)+lower;

       //         printf("lat %f lon %f", lon64, lat64);
        }
        if(intr_free){
                free(interleaved);
        }
...
Persistent<FunctionTemplate> GeoPoint::constructor;

void GeoPoint::Init(Handle<Object> target) {
    HandleScope scope;

    Local<FunctionTemplate> tpl = FunctionTemplate::New(New);
    Local<String> name = String::NewSymbol("GeoPoint");
   ...
   // NODE_SET_PROTOTYPE_METHOD(constructor, "value", Value);
    ...
    NODE_SET_PROTOTYPE_METHOD(constructor, "lon", Lat);
...
Handle<Value> GeoPoint::New(const Arguments& args) {
    HandleScope scope;
    ...
    GeoPoint* obj = new GeoPoint(args[0]->ToNumber()->Value(), args[1]->ToNumber()->Value());
    obj->Wrap(args.This());
...
Handle<Value> GeoPoint::Lat(const Arguments& args) {
    HandleScope scope;

    // Retrieves the pointer to the wrapped object instance.
    GeoPoint* obj = ObjectWrap::Unwrap<GeoPoint>(args.This());

    return scope.Close(Number::New(obj->lat_));
}

Handle<Value> GeoPoint::Lon(const Arguments& args) {
    HandleScope scope;

    // Retrieves the pointer to the wrapped object instance.
    GeoPoint* obj = ObjectWrap::Unwrap<GeoPoint>(args.This());

    return scope.Close(Number::New(obj->lon_));
}


Handle<Value> geohasherjs_encode_dec(const Arguments& args) {
    HandleScope scope;
    if (args.Length() < 2) {
     return ThrowException(
                Exception::TypeError(String::New("Arguments required: lat, lon"))
            );
    }

    Local<Number> theLat = args[0]->ToNumber();
   // double thelat = theLat->Value();

    Local<Number> theLon = args[1]->ToNumber();
   // double thelon = theLon->Value();

    uint64_t lat64, lon64;
    if(!double_to_i64( theLat->Value()/90.0, &lat64) || !double_to_i64(theLon->Value()/180.0, &lon64)){
                 return ThrowException(
                            Exception::TypeError(String::New("invalid params"))
                        );
        }
    uint16_t interleaved[8];
//    char lr[27];
    ...
    uint64_t done = ((unsigned LONG_LONG)interleaved[0]<<48) + ((unsigned LONG_LONG)interleaved[1]<<32) + ((unsigned LONG_LONG)interleaved[2]<<16) + (unsigned LONG_LONG)interleaved[3];
    char buffer [150];
    //printf("hi: %llu\0", done);
    int n=sprintf (buffer, "%llu",done);
    if (n < 0 ) {
          return ThrowException(
...
Handle<Value> geohasherjs_decode_dec(const Arguments& args) {
    //HandleScope scope;
    if (args.Length() < 1) {
     return ThrowException(
                Exception::TypeError(String::New("Arguments required: geohash decimal string, callback"))
            );
    }
 ...
 // There's no ToFunction(), use a Cast instead.
    ...
    // Local<Number> theGeo = args[0]->ToNumber();

        Local<String> theGeoString = args[0]->ToString();

       // printf("%d", theGeoString->Length());

        char buffer[150];
        memset(buffer, 0, 150);
        theGeoString->WriteAscii(buffer,0,theGeoString->Length());

      //  printf("%s\n", buffer);


       // atoi
        const unsigned long long lo = strtoull(buffer, NULL, 10);
   ...
   //TODO handle errors
      ...
      const unsigned argc = 3;
        Local<Value> argv[argc] = {
            ...
            Local<Value>::New(Number::New(tlat*90)),
            Local<Value>::New(Number::New(tlon*180))
        };
...
Handle<Value> geohasherjs_encode(const Arguments& args) {
    HandleScope scope;
    if (args.Length() < 2) {
     return ThrowException(
                Exception::TypeError(String::New("Arguments required: lat, lon, precision"))
            );
    }
    ...
    Local<Integer> thePrecision=args[1]->ToInteger();
    int theprec = thePrecision->IntegerValue();

//TODO handle errors

    char buffer[50];
    geohash_encode_impl(thelat,thelon,buffer, theprec );
...
Handle<Value> geohasherjs_decode(const Arguments& args) {
    if (args.Length() < 1) {
     return ThrowException(
                Exception::TypeError(String::New("Arguments required: geohash string, callback"))
            );
    }
...
//TODO handle errors
    ...
    Local<Function> callback = Local<Function>::Cast(args[1]);
        ...
        theGeoString->WriteAscii(buffer,0,theGeoString->Length());
            ...
            double outLat;
            double outLon;
            geohash_decode(buffer, theGeoString->Length(), &outLat, &outLon);

        const unsigned argc = 3;
        Local<Value> argv[argc] = {
            Local<Value>::New(Null()),
            Local<Value>::New(Number::New(outLat)),
            Local<Value>::New(Number::New(outLon))
        };
...
Handle<Value> geohasherjs_neighbor(const Arguments& args) {
 return ThrowException(
                Exception::TypeError(String::New("Not Implemented"))
            );

    /*  HandleScope scope;
        if (args.Length() < 2) {
         return ThrowException(
                    Exception::TypeError(String::New("Arguments required: geohash, precision"))
                );
        }

       Local<String> theGeoString = args[0]->ToString();
       char hashcode[150];
       memset(hashcode, 0, 150);
       theGeoString->WriteAscii(hashcode,0,theGeoString->Length());


        Local<Integer> theDirection=args[1]->ToInteger();
        int theprec = thePrecision->IntegerValue();
        ...
        if(buffer==NULL){
            Exception::TypeError(String::New("No Memory"))
        }

        if((ret = geo_neighbors_impl(hashcode, buffer, buffer_sz, &string_count)) != GEOHASH_OK){
           Exception::TypeError(String::New("Error finding neighbor"))
        }

        Local<String> returnString = String::New(buffer);
         free(buffer);
             return scope.Close(returnString);*/
...
void RegisterModule(v8::Handle<v8::Object> target) {
    ...
    GeoPoint::Init(target);
    target->Set(String::NewSymbol("encode_dec"),
            FunctionTemplate::New(geohasherjs_encode_dec)->GetFunction());

    target->Set(String::NewSymbol("encode"),
                FunctionTemplate::New(geohasherjs_encode)->GetFunction());

    target->Set(String::NewSymbol("decode"),
                FunctionTemplate::New(geohasherjs_decode)->GetFunction());

   // target->Set(String::NewSymbol("decode_dec"),
         //       FunctionTemplate::New(geohasherjs_decode_dec)->GetFunction());

    target->Set(String::NewSymbol("decode_bbox"),
                FunctionTemplate::New(geohasherjs_encode)->GetFunction());

    target->Set(String::NewSymbol("neighbor"),
                FunctionTemplate::New(geohasherjs_neighbor)->GetFunction());
...
NODE_MODULE(geohasherjs, RegisterModule);
